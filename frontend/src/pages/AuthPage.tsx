...
import { User, Lock, Eye, EyeOff, Building, Briefcase } from 'lucide-react';
...
export const AuthPage: React.FC = () => {
  ...
  const handleGoogleAccountSelect = (selectedName: string, selectedEmail: string, selectedBusiness: string) => {
    setIsLoading(true);
    setShowGoogleModal(false);
    setTimeout(() => {
      login(selectedEmail, 'MSME', selectedName, selectedBusiness);
      setIsLoading(false);
      navigate('/dashboard');
    }, 800);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (isSignUp) {
        login(email, role, fullName, businessName);
      } else {
        login(email, role, email);
      }
      setIsLoading(false);
      navigate('/dashboard');
    }, 600);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <motion.div
        ...
        className="w-full max-w-[400px] bg-white rounded-[32px] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.08)] z-10 my-8"
      >
        {/* Logo & Header */}
        <div className="text-center mb-6">
          <motion.div 
            className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-4 border border-blue-100 shadow-sm overflow-hidden p-2"
            animate={{ 
              y: [0, -8, 0],
              rotate: [0, 5, -5, 0],
          ...
          <p className="text-xs text-slate-500 mt-1.5 font-medium">
            {isSignUp ? 'Create your business account to begin' : 'Welcome back! Please sign in to continue'}
          </p>
          ...
          {isSignUp && (
            <>
              ...
              <div className="space-y-1.5 text-left">
                <label className="text-[11px] font-bold text-slate-700 ml-1">Full Name (Username)</label>
              ...
              {/* Business Name Input for Registration */}
              <div className="space-y-1.5 text-left">
                <label className="text-[11px] font-bold text-slate-700 ml-1">Business Name</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
        ...
        {/* Footer Link (Toggle Sign up / Login) */}
        <div className="mt-8 text-center text-[11px] font-medium text-slate-500">
          {isSignUp ? (
            <>Already have an account? <button onClick={() => setIsSignUp(false)} className="text-blue-600 font-bold hover:underline">Sign In here</button></>
          ) : (
            <>Don't have an account? <button onClick={() => setIsSignUp(true)} className="text-blue-600 font-bold hover:underline">Create Account</button></>
          )}
        </div>
      </motion.div>
        ...
        {showGoogleModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
              ...
              <h3 className="text-xl font-medium text-center text-slate-800 mb-1">Sign in</h3>
              <p className="text-sm text-center text-slate-600 mb-6">Choose an account to continue to Nool Credit</p>

              <div className="space-y-2">
                {/* Account 1 */}
                <button
                  onClick={() => handleGoogleAccountSelect('Ramesh Kumar', 'ramesh.kumar@gmail.com', 'Ramesh Textiles')}
                  className="w-full flex items-center gap-3 p-3 hover:bg-slate-50 rounded-xl transition-colors border border-transparent hover:border-slate-200 text-left"
                >
                  <div className="w-10 h-10 rounded-full bg-purple-600 text-white flex items-center justify-center font-bold text-lg">R</div>
                  <div>
                    <div className="text-sm font-medium text-slate-900">Ramesh Kumar</div>
                    <div className="text-xs text-slate-500">ramesh.kumar@gmail.com</div>
                  </div>
                </button>
                
                {/* Account 2 */}
                <button
                  onClick={() => handleGoogleAccountSelect('Lakshmi Devi', 'lakshmi.devi@gmail.com', 'Sri Lakshmi Knits')}
                  className="w-full flex items-center gap-3 p-3 hover:bg-slate-50 rounded-xl transition-colors border border-transparent hover:border-slate-200 text-left"
                >
                  <div className="w-10 h-10 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold text-lg">L</div>
                  <div>
                    <div className="text-sm font-medium text-slate-900">Lakshmi Devi</div>
                    <div className="text-xs text-slate-500">lakshmi.devi@gmail.com</div>
                  </div>
                </button>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-slate-200 flex justify-end">
                <button 
                  ...
                  className="text-sm text-blue-600 font-medium hover:bg-blue-50 px-4 py-2 rounded-lg"
                >
                  Cancel
                </button>
              </div>
